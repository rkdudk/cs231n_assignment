...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    이 문제는 주어진 범위 (start부터 stop까지) 내에서 10의 배수들을 찾아서 
    torch.float64 타입의 텐서로 반환하는 함수 작성 문제입니다. 만약 범위 내에 10의 배수가 없다면 빈 텐서를 반환해야 합니다.
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    first = ((start+9)//10)*10 # 처음 10배수
    last = (stop//10)*10 # 마지막 10배

    x = torch.arange(first, last+1, 10, dtype=torch.float64) if first <= last else torch.empty(0, dtype=torch.float64)
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    헷갈렸던 점:
        last_row - 단일 인덱스로 슬라이싱 (e.g., x[-1, :] 또는 x[:, 2]) → 차원 축소 (1차원 반환)
        third_col - 범위 슬라이싱 (e.g., x[-1:, :] 또는 x[:, 2:3]) → 차원 유지 (2차원 반환)
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    **permute 계산 과정**
    맨 처음 [block, row, col] 을 [row, block, col]로 변환한다.
    - 규칙을 찾고, permute 추론하기.
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mu_j = x.sum(dim=0) / x.shape[0] # shape(N,) - 평균 구함
    """
    x:   (M, N)
    mu:     (N,)  → 해석: (1, N) → 자동으로 M개의 row로 broadcast됨
    """
    sigma_j = ((((x - mu_j)**2).sum(dim=0)) / (x.shape[0]-1)).sqrt()
...
def mm_on_gpu(x: Tensor, w: Tensor) -> Tensor:
    ...
    y_gpu = x_gpu.mm(w_gpu)  # or x_gpu @ w_gpu
    y = y_gpu.cpu()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
